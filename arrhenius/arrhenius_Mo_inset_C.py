...
fig, axs = plt.subplots(dpi=450, nrows=1, ncols=3, figsize=(6.5, 3.5), sharex=False, sharey=False, layout="constrained")
...
k, kerr = plot_rxn_over_time(start, time_per_frame_ps, axs[1], temps = temps, rxns = ["H-H.True.com.npy", "H-H.True.dis.npy"], linestyles = ["solid", "dotted"])

print("Combinations")
plot_arrhenius(k[0], kerr[0], temps, axs[2], colors, "solid", "v")
print("Dissocations")
plot_arrhenius(k[1], kerr[1], temps, axs[2], colors, "dotted", "^")


#axins = inset_axes(axs[1], width =1.0 , height = 1.0, loc = 2, borderpad=2.5)
axins = inset_axes(axs[1], width =1.0 , height = 1.0, bbox_to_anchor=(.2, .5, .5, .5), bbox_transform=axs[1].transAxes, loc = 2   )

buffer = 500
axs[1].set_ylim((0-buffer, 13000 + buffer))
k, kerr = plot_rxn_over_time(start, time_per_frame_ps, axins, temps = temps, rxns = ["H-H.False.com.npy", "H-H.False.dis.npy"], linestyles = ["solid", "dotted"])
...
axs[1].set_ylabel(r"Count", labelpad = -1)
axs[1].set_xlabel(r"$t$ (ps)")

axs[2].set_ylabel(r"ln($k$ (ps$^{-1}$))")
axs[2].set_xlabel(r"1/$k_{B}T$ (1/eV)")
