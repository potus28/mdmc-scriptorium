...
select_atom_indices = np.load(sys.argv[3]) # ../../indices/zeolite_SnO_atoms_indices.npy
coordinating_atom_symbol = sys.argv[4] # O

start_frame = int(sys.argv[5]) # 30000
kbond = float(sys.argv[6]) # 1.46
...
@njit
def calc_angle_selections(r, central_atom, coord_atoms, select_atoms, hmat):

    ncoord = len(coord_atoms)
    nselect = len(select_atoms)
    nangles = ncoord * nselect
    angle = np.zeros(nangles)
    rcenter = r[central_atom]
...
coord_atom_indices = np.array(
        [atom.index for atom in traj[0] if atom.symbol == coordinating_atom_symbol and atom.index not in select_atom_indices]
)
...
for atoms in traj[start_frame:]:
    ...
    for i, center in enumerate(center_atom_indices):
        coord = []
        select = select_atom_indices[i]
        for c_idx in coord_atom_indices:
            ...
            if len(coord) == 0:
                tetrahedrals[str(frame)].append([])
            else:
                tetrahedrals[str(frame)].append(calc_angle_selections(r, center, coord, select, hmat))
...
np.save("tetrahedral-otf-select.npy", data)
